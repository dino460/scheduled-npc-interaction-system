...
pub const VERSION_NUMBER: &str = "0.3.2.1";
            ...
            if path.contains(&(i, j)) {
                print!(
                    ...
                    (matrix[i][j].to_string() + " ")
                        .to_string()
                        .truecolor(
                            MATRIX_COLOR_LUT[matrix[i][j]].0,
                            MATRIX_COLOR_LUT[matrix[i][j]].1,
                            MATRIX_COLOR_LUT[matrix[i][j]].2
                        )
                        .on_purple()
                )
                ...
                print!(
                    "{}",
                    (matrix[i][j].to_string() + " ").to_string().truecolor(
                        MATRIX_COLOR_LUT[matrix[i][j]].0,
                        MATRIX_COLOR_LUT[matrix[i][j]].1,
                        MATRIX_COLOR_LUT[matrix[i][j]].2
                    )
...
pub fn print_title() {
    ...
    println!(
        ...
        "Scheduled NPC Interaction System "
            ...
            .bold(),
        "v" /*.italic()*/
            .bright_red(), /*.underline()*/
        VERSION_NUMBER /*.italic()*/
            .bright_red(), /*.underline()*/
...
pub fn print_greeting_tutorial() {
    ...
    println!(
        "{}{:?}",
        " - Generate new random map      : ".italic().dimmed(),
        GENERATE_INPUT_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Save current map to file     : ".italic().dimmed(),
        SAVE_INPUT_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Load map from file           : ".italic().dimmed(),
        LOAD_INPUT_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Find path between two points : ".italic().dimmed(),
        FIND_INPUT_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Set map size (NxN matrix)    : ".italic().dimmed(),
        SET_SIZE_INPUT_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Print current map            : ".italic().dimmed(),
        PRINT_INPUT_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Show all commands            : ".italic().dimmed(),
        SHOW_ALL_COMMANDS_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Reset screen to greeting     : ".italic().dimmed(),
        RESET_SCREEN_INPUT_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Clear console                : ".italic().dimmed(),
        CLEAR_INPUT_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Exit program                 : ".italic().dimmed(),
        EXIT_INPUT_OPTIONS
    );
    println!("========================================================================");
    // println!();
    ...
    println!(
        "\" {}{}{}{}{} \"",
        "[show map on gen|show commands|matrix size] "
            .italic()
            .green(),
        "(".italic().cyan(),
...
pub fn print_all_commands() {
    ...
    println!(
        "{}{:?}",
        " - Toggle showing map on gen    : ".italic().dimmed(),
        HIDE_GEN_INPUT_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Toggle showing commands      : ".italic().dimmed(),
        HIDE_COMMAND_OPTIONS
    );
    println!(
        "{}{:?}",
        " - Reset screen to greeting     : ".italic().dimmed(),
