...
fn main() -> std::io::Result<()> {
    ...
    let mut world: World = World {
        paths: (0..matrix_size)
            .map(|_| (0..matrix_size).map(|_| 0).collect())
            .collect(),
        smoothed_paths: (0..matrix_size)
            .map(|_| (0..matrix_size).map(|_| 0).collect())
            .collect(),
        jobs: (0..matrix_size)
            .map(|_| (0..matrix_size).map(|_| 0).collect())
    ...
    loop {
        let hide_gen_indicator = if print_generation_result {
            print_generation_result
                .to_string()
                .truecolor(
                    WARNING_COLOR_LUT[2].0,
                    WARNING_COLOR_LUT[2].1,
                    WARNING_COLOR_LUT[2].2,
                )
                .to_string()
        } else {
            print_generation_result
                .to_string()
                .truecolor(
                    WARNING_COLOR_LUT[0].0,
                    WARNING_COLOR_LUT[0].1,
                    WARNING_COLOR_LUT[0].2,
                )
                .to_string()
        };
        ...
        if GENERATE_INPUT_OPTIONS.contains(&input.to_lowercase().as_str()) {
            ...
            let now = Instant::now();
            process_world_generation(
                &mut world,
                matrix_size,
                weight_distribution_weights,
                path_distribution_weights,
                job_distribution_weights,
            );
            ...
            println!(
                "{} {} {} s",
                INFO_SYMBOL.truecolor(
                    WARNING_COLOR_LUT[2].0,
                    WARNING_COLOR_LUT[2].1,
                    WARNING_COLOR_LUT[2].2
                ),
                "Generated successfully in".truecolor(
                    WARNING_COLOR_LUT[2].0,
                    WARNING_COLOR_LUT[2].1,
                    WARNING_COLOR_LUT[2].2
                ),
            ...
            source.0 = match read_input::<usize>(
                input.clone(),
                "\t",
                "\t> Source X coordinate value:"
                    .yellow()
                    .to_string()
                    .as_str(),
            ) {
            ...
            if source.0 >= matrix_size {
                println!(
                    ...
                    WARNING_SYMBOL
                        .truecolor(
                            WARNING_COLOR_LUT[1].0,
                            WARNING_COLOR_LUT[1].1,
                            WARNING_COLOR_LUT[1].2
                        )
                        .blink(),
                    "Value out of bounds".truecolor(
                        WARNING_COLOR_LUT[1].0,
                        WARNING_COLOR_LUT[1].1,
                        WARNING_COLOR_LUT[1].2
                    ),
            ...
            source.1 = match read_input::<usize>(
                input.clone(),
                "\t",
                "\t> Source Y coordinate value:"
                    .yellow()
                    .to_string()
                    .as_str(),
            ) {
            ...
            destination.0 = match read_input::<usize>(
                input.clone(),
                "\t",
                "\t> Destination X coordinate value:"
                    .yellow()
                    .to_string()
                    .as_str(),
            ) {
            ...
            destination.1 = match read_input::<usize>(
                input.clone(),
                "\t",
            ...
            let mut dest = Point {
                i: Some(destination.0 as i32),
                j: Some(destination.1 as i32),
                distance: None,
                previous: None,
            };

            let use_weights: bool = match read_input_to_bool(
                input.clone(),
                "\t",
                &("\t> Use weights? [y, yes | n, no]".yellow().to_string()),
            ) {
                Ok(value) => value,
                _ => continue,
            };
            ...
            let use_diagonals: bool = match read_input_to_bool(
                input.clone(),
                "\t",
                &("\t> Compute diagonal movement? [y, yes | n, no]"
                    .yellow()
                    .to_string()
                    + " (High performance impact)".red().to_string().as_str()),
            ) {
                Ok(value) => value,
                _ => continue,
            };
            ...
            let path_exists = path_finding::pathfinder(
                ...
                &world.weights,
                &Point {
                    i: Some(source.0 as i32),
                    j: Some(source.1 as i32),
                    distance: None,
                    previous: None,
                },
                &mut dest,
            ...
            println!(
                ...
                destination.1,
                if path_exists {
                    path_exists.to_string().truecolor(
                        WARNING_COLOR_LUT[2].0,
                        WARNING_COLOR_LUT[2].1,
                        WARNING_COLOR_LUT[2].2,
                    )
            ...
            while path_start != None {
                path.push((
                    path_start.clone().unwrap().i.unwrap() as usize,
                    path_start.clone().unwrap().j.unwrap() as usize,
                ));
                path_start = path_start.unwrap().previous.clone();
            }

            gui::print_matrix_with_path(&world.world, destination, source, &path);
            ...
            matrix_size = read_input::<usize>(
                input.clone(),
                "",
                "> Type matrix size (matrix is always NxN)",
            )
            .unwrap(); /*input.parse::<usize>().unwrap()*/
            ...
            let now = Instant::now();
            let benchmarks: Vec<Benchmark> = benchmark_all(
                &mut world,
                2000,
                50,
                51,
            ...
            println!(
                ...
                input.bold(),
                "' command does not exist.".truecolor(
                    WARNING_COLOR_LUT[1].0,
                    WARNING_COLOR_LUT[1].1,
                    WARNING_COLOR_LUT[1].2
                )
    ...
    <T as std::str::FromStr>::Err: std::fmt::Debug,
{
    ...
    print!(
        "{}({}): ",
        at_text,
        std::any::type_name::<T>().to_string().italic().dimmed()
    );
...
fn read_input_to_bool(
    ...
    print!(
        "{}({}): ",
        at_text,
        std::any::type_name::<bool>().to_string().italic().dimmed()
    );
